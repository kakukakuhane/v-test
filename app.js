...
// ===== フィルタ =====
function getList() {
  let base = mode === "wrong" ? wrongQuestions : quizData;
...
function render() {
  ...
  q.choices.forEach((c, i) => {
    ...
    if (state.answered) {
      ...
      if (i === state.selected && i !== q.correct)
...
function answer() {
  ...
  state.answered = true;
  state.isCorrect = state.selected === q.correct;

  if (!state.isCorrect && !wrongQuestions.includes(q)) {
...
// ===== スコア =====
function getScore() {
  return states.filter(s => s.isCorrect).length;
}
